...
def Candidate_prediction(Gender, Employee_Group, Country, Business_Unit,Job_Classification, Supervisor,Experience, Age ):
    test_data = pd.DataFrame([[Gender, Employee_Group, Country, Business_Unit,Job_Classification, Supervisor,Experience  ,Age ]],
                         columns=['Gender','Employee_Group','Country','Business_Unit','Job_Classification','Supervisor','Experience  ','Age '])
    prediction=classifier.predict(test_data)
    print(prediction)
    if (prediction[0] == 0):
        return 'The person is likely to stay longer time in Zalaris  '
    else:
        return 'The person is not likely to stay longer time in Zalaris  '
    
encoder_excel_path = "LE code.xlsx"
...
def main():
    ...
    Experience = st.sidebar.number_input('Insert Experience ')
    ...
    if st.button('Candidate Result'):
        Candidate = Candidate_prediction(Gender, Employee_Group, Country, Business_Unit,Job_Classification, Supervisor, Experience  ,Age )
